...
def verify_otp():
    ...
    try:
        ...
        payload = {
            'user_id': user.id,
            'email': user.email,
            'role': user.role.value if user.role else None, # Include role or None
...
def google_callback():
    try:
        ...
        # Redirect user to frontend, setting the JWT as a cookie
        # The frontend will then use this cookie for subsequent API calls
        ...
        # Redirect to a generic error page or login page on failure
